...
import { solanaConnection, TX_FEE } from "../config/config"
import { getAssociatedTokenAddress, getMint } from "@solana/spl-token"
import { ApiPoolInfoV4, buildSimpleTransaction, jsonInfo2PoolKeys, Liquidity, LIQUIDITY_STATE_LAYOUT_V4, LiquidityPoolKeys, LOOKUP_TABLE_CACHE, Market, MARKET_STATE_LAYOUT_V3, Percent, SPL_ACCOUNT_LAYOUT, SPL_MINT_LAYOUT, Token, TOKEN_PROGRAM_ID, TokenAccount, TokenAmount, TxVersion } from "@raydium-io/raydium-sdk"
...
export async function getBuyTx(wallet: Keypair, baseMint: PublicKey, quoteMint: PublicKey, amount: number, targetPool: string) {
...
export async function getSellTx(wallet: Keypair, baseMint: PublicKey, quoteMint: PublicKey, amount: string, targetPool: string) {
    try {
